...
def get_counterfactual_data_single_m(args):
    ...
    for ri in range(1, args.counter_run + 1):
        ...
        if args.model_flag in ["m2", "m4"]:
            ...
            # pick up the estimates from Rri_x.csv
            ...
            for gi in group_list:
                if args.counter_g in gi: # FW and FB
                    x_shifts[gi] = 0
                else: # MW and MB
            ...
            cur_df["X_shift"] = cur_df["GR"].apply(lambda x: x_shifts[x])
            cur_df["X_count"] = cur_df["X"] + cur_df["X_shift"]
            # get new value assuming candidate is F (i.e., counter_g_base)
        ...
        if args.model_flag in ["m1", "m3"]:
            ...
            for gi in group_list:
                ...
                if args.hidden_g in gi: # for MB and FB
            ...
            med_weight = pd.read_csv(data_name + "_" + args.other_g + "_med.csv")["Estimate"][0] # picks ACME

        cur_df["Y_shift"] = cur_df["GR"].apply(lambda x: y_shifts[x])

        cur_df["Y_count"] = cur_df["Y"] + cur_df["Y_shift"] + med_weight * cur_df["X_shift"]

        if args.model_flag in ["m2", "m4"]:

            cur_df["Y_count_resolve"] = cur_df["Y"] + cur_df["Y_shift"]
