...
def eval_counter_results(args):
    ...
    for ri, fi in enumerate(all_files):
        ...
        seti_quotas = get_quotas_count(count_df) # seti_quotas stores the proportion (in (0,1)) of each group in entire set

        for rank_all in args.rankings.split(","):
            if "__" in rank_all:
                train_ranki = rank_all.split("__")[0] ## for learning to rank only
            else:
            ...
            for ki in k_list:
                ...
                if args.measure == "rKL":
                    if args.data_flag == "cm": # random permutate for COMPAS to avoid bias due to sorting with ties
                        ...
                        assert(False) # we never go here (for synthetic)
                    else:
                        all_row.append(compute_rKL(sort_df, orig_df, group_col=args.group_col))


                if args.measure == "raw_counts" or args.measure == "get_util" :
                    all_row.append(0)
                ...
                for gi in list(orig_df[args.group_col].unique()):
                    ...
                    if args.measure == "rKL": # not applicable to group
                        gi_row.append(-1)

                    if args.measure != "raw_counts" and args.measure != "get_util":
                        res_df.loc[res_df.shape[0]] = gi_row

                if args.measure == "raw_counts":
                    counts = dict(sort_df['R'].value_counts(normalize=False))
                    for gi in ['W', 'B']:
                        gi_row = res_row + [gi]
                        if gi in counts:
                            gi_row.append(counts[gi])
                        else:
                            gi_row.append(0)
                        res_df.loc[res_df.shape[0]] = gi_row

                    counts = dict(sort_df['GR'].value_counts(normalize=False))
                    for gi in list(orig_df[args.group_col].unique()):
                        gi_row = res_row + [gi]
                        if gi in counts:
                            gi_row.append(counts[gi])
                ...
                if args.measure == "get_util":
                    import numpy as np

                    counts = dict(sort_df['R'].value_counts(normalize=False))

                    val = np.sum(sort_df['Y'])
                    gi_row = res_row + ["VAL"]
                    gi_row.append(val)
                    res_df.loc[res_df.shape[0]] = gi_row
        if args.verbose:
