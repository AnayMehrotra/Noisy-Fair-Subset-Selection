...
def get_quotas_count(_df):
    # returns quotas as fraction (in (0,1))
    res_dict = {}
...
def get_quotas_df(_df, _sort_col, _eval_k, quotas_budget, quotas_on="GR"):
    ...
    quotas_k = {x: int(_eval_k*quotas_budget[x]) for x in group_list} ## TODO: Put a ceil here
    ...
    return sep_d # final dataset with quotas


def get_sort_df(_sort_col, _df, _k, quotas_max=None):
    if "quotas" in _sort_col:
        eval(debug("_sort_col, _sort_col[0]"))
